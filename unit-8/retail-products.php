...
if ($row = $stmt->fetch()) {
    do {
        ...
        $productStatusHTML = $productStatus ? "<p class=\"productStatus\">$productStatus</p>" : '';
        $productLowInventoryCSS = ($productInStock < 10) ? 'productLowInventory' : '';

        $queryResultOutput .= <<<END
            <div class="productBlock">
                <div class="productImage">
                    <image src="productImages/$productImage">
                </div>
                <p class="productName">$productName</p>
                <p class="productDesc">$productDescription</p>
                ...
                $productStatusHTML       
                <p class="productInventory $productLowInventoryCSS">$productInStock In Stock!</p>
            </div>
        END;
    } while ($row = $stmt->fetch());
} else {
    $queryResultOutput = "No products found.";
}
?>
