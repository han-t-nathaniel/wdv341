...
if (!$row = $stmt->fetch()) {
    ...
    $queryResultOutput .= "<th class=\"options-column\">Options</th>";
    $queryResultOutput .= "</tr></thead><tbody>";
    do {
        $formattedDescription = $row["description"];
        if (strlen($formattedDescription) > 50) {
            $formattedDescription = substr($formattedDescription, 0, 50) . "...";
        }
...
<head>
    ...
    <title>WDV 341 | Manage Events</title>
...
<nav>
    <div class="site-logo">
        <a href="/" aria-label="Return to the home page">
            <lottie-player role="img" aria-label="NGH logo" autoplay mode="normal"
                           src="/img/ngh-logo.json"></lottie-player>
        </a>
    </div>
    <div class="nav-links">
